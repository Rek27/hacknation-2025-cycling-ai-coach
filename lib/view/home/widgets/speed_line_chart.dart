  ...
  Widget build(BuildContext context) {
    ...
    final List<FlSpot> spots = <FlSpot>[
      for (int i = 0; i < last.length; i++)
        FlSpot(i.toDouble(), last[i].averageSpeedKmh)
    ];

    final double avg = last.isEmpty
        ? 0
        : last.fold<double>(0, (p, e) => p + e.averageSpeedKmh) / last.length;

    return ChartCard(
      title: 'Average speed (km/h)',
      subtitle: 'Average: ${avg.toStringAsFixed(1)} km/h',
      child: LineChart(
        LineChartData(
          ...
          titlesData: FlTitlesData(
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                showTitles: true,
                interval: (last.length / 6).clamp(1, 6).toDouble(),
                getTitlesWidget: (value, meta) => Text('${value.toInt() + 1}',
                    style: theme.textTheme.bodySmall),
              ),
            ),
